...
local RangoADM = {}
addEvent('resentRangoADM', true)
addEventHandler('resentRangoADM', root, function(string) RangoADM[source] = string end)
...
addEventHandler("onClientRender", getRootElement(), function()
	...
	for _, player in ipairs(Element.getAllByType("player")) do
		if RangoADM[player] and RangoADM[player]:len() > 0 then
			local hex = "#"..RColor[RangoADM[player]]
			local bx,by,bz = getPedBonePosition(player, 8)
			local bsx, bsy = getScreenFromWorldPosition( bx,by,bz+0.25 )
			if bsx and bsy then
				local x1, y1, z1 = getElementPosition(localPlayer)
				if player ~= localPlayer then
				local x2, y2, z2 = getElementPosition(player)
				local pdistance = getDistanceBetweenPoints3D ( x1,y1,z1,x2,y2,z2 )
					if pdistance <= 15 then
						dxDrawBorderedText ( hex.."["..player:getData("nombre:staff").."]", bsx, bsy, bsx, bsy, tocolor(0,255,0,255), 1.2, "default-bold", "center", "center", false, false, false, true)
					end
				else
					dxDrawBorderedText ( hex..""..player:getData("nombre:staff").."", bsx, bsy, bsx, bsy, tocolor(0,255,0,255), 1.2, "default-bold", "center", "center", false, false, false, true)
				end
	...
	for player in pairs(nametags) do
		if player:getData("inv",false) or player:getData("Admin:Disponible",false) then
		else
		while true do
			...
			if pdistance <= 15 then
				...
				dxDrawBorderedText ( "".._getPlayerNameR(player:getName()).." #FFFFFF( ID: #FF3300"..idPlayer.."#FFFFFF ) "..textolag.."", sx, sy+35, sx, sy+35, tocolor(r,g,b,255), 1.2, "default-bold", "center", "center")
				dxDrawRectangle(drawX-3, drawY-3+70, width, height, tocolor(0, 0, 0, 255), false)
				dxDrawRectangle(drawX, drawY+70, width-6, height-6, tocolor(55, 55,55, 255), false)
				...
				local r, g = math.max(math.min(p + 255*health + 255, 255), 0), math.max(math.min(p + 765*health, 255), 0)
 				...
 				if vida then
 					dxDrawRectangle(drawX, drawY+70, (width - 6)*health, height - 6, tocolor(255, 3, 3, 255))
 				end
 				--
