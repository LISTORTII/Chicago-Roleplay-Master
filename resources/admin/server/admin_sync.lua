...
function aSynchCoroutineFunc( type, data, typeOfTag, banSearchTag )
	...
	elseif ( type == "resources" ) then
		...
		for id, resource in ipairs(resourceTable) do
			...
			tableOut[id]["fullName"] = getResourceInfo(resource, "name") or "desconocido"
			tableOut[id]["author"] = getResourceInfo(resource, "author") or "desconocido"
			tableOut[id]["version"] = getResourceInfo(resource, "version") or "desconocida"
			...
			if ( getTickCount() > tick + 100 ) then
				...
				if (antiCorMessageSpam == 0) then
					outputChatBox("Por favor espere, lista de resources cargando... No intente actualizar.", source, 255, 255, 0)
				end
	...
	elseif ( type == "admins" ) then
		for id, player in ipairs(getElementsByType("player")) do
			if aPlayers[player] then
				...
				tableOut[player]["groups"] = "ninguno"
