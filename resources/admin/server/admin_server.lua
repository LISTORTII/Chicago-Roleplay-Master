...
function notifyPlayerLoggedIn(player)
	outputChatBox ( "#ff0303[STAFF] #ffffffPresiona 'p' para abrir el panel de admin", player,255,255,255,true )
	local unread = 0
	for _, msg in ipairs ( aReports ) do
		unread = unread + ( msg.read and 0 or 1 )
	end
	if unread > 0 then
		outputChatBox( unread .. " mensajes no leídos" .. ( unread==1 and "" or "s" ), player, 255, 0, 0 )
	end
end
...
function aPlayerInitialize ( player )
	...
	if ( not isValidSerial ( serial ) ) then
		outputChatBox ( "ERROR: "..getPlayerName ( player ).." - serial inválida." )
...
addCommandHandler ( "register", function ( player, command, arg1, arg2 )
	...
	if ( password ~= nil ) then
		if ( string.len ( password ) < 4 ) then
			outputChatBox ( "registrar: - la contraseña debe tener al menos 4 caracteres.", player, 255, 100, 70 )
		elseif ( addAccount ( username, password ) ) then
			outputChatBox ( "¡Registrado con éxito! Usuario: '"..username.."', Contraseña: '"..password.."'(Recuérdala)", player, 255, 100, 70 )
		...
		elseif ( getAccount ( username ) ) then
			outputChatBox ( "registrar: - ya existe una cuenta con este nombre.", player, 255, 100, 70 )
		else
			outputChatBox ( "Error desconocido", player, 255, 100, 70 )
		end
	else
		outputChatBox ( "registrar: - el comando es 'register [<usuario>] <constraseña>'", player, 255, 100, 70 )
	end
end )
...
addCommandHandler ( "unregister", function ( player, command, arg1, arg2 )
	...
	outputChatBox ( "Cancelando el registro de '"..username.."' "..result, player, 255, 100, 70 )
	outputServerLog ( "ADMIN: "..getAdminNameForLog ( player ).." cancelando el registro de '"..username.."' "..result.." (IP: "..getPlayerIP(player).."  Serial: "..getPlayerSerial(player)..")" )
end )
...
function aAction ( type, action, admin, player, data, more )
	if ( aLogMessages[type] ) then
		function aStripString ( string )
			...
			string = string.gsub ( string, "$by_admin_4all", isAnonAdmin4All( admin )    and "" or " por " .. getPlayerName ( admin ) )
			string = string.gsub ( string, "$by_admin_4plr", isAnonAdmin4Victim( admin ) and "" or " por " .. getPlayerName ( admin ) )
...
addEventHandler ( "aTeam", _root, function ( action, name, r, g, b )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		if ( action == "createteam" ) then
			...
			if ( not success ) then
				action = nil
				outputChatBox ( "El team \""..name.."\" no pudo ser creado.", source, 255, 0, 0 )
			end
	...
	outputChatBox ( "Acceso denegado para '"..tostring ( action ).."'", source, 255, 168, 0 )
	return false
end )
...
addEventHandler ( "aAdmin", _root, function ( action, ... )
	...
	if ( action == "password" ) then
		action = nil
		if ( not arg[1] ) then outputChatBox ( "Error - contraseña faltante.", source, 255, 0, 0 )
		elseif ( not arg[2] ) then outputChatBox ( "Error - nueva contraseña faltante.", source, 255, 0, 0 )
		elseif ( not arg[3] ) then outputChatBox ( "Error - confirmar contraseña.", source, 255, 0, 0 )
		elseif ( tostring ( arg[2] ) ~= tostring ( arg[3] ) ) then outputChatBox ( "Error - las contraseñas no coinciden.", source, 255, 0, 0 )
		else
			...
			if ( account ) then
				...
				outputChatBox ( "Error - contraseña inválida.", source, 255, 0, 0 )
			end
		end
	...
	elseif ( action == "aclcreate" ) then
		...
		if ( ( name ) and ( string.len ( name ) >= 1 ) ) then
			...
			outputChatBox ( "Error - nombre "..arg[1].." inválido", source, 255, 0, 0 )
		end
		...
		if ( arg[3] ) then
			...
			if ( arg[1] == "object" ) then
				...
				if ( not aclGroupAddObject ( group, object ) ) then
					action = nil
					outputChatBox ( "Error añadiendo objeto '"..tostring ( object ).."' al grupo '"..tostring ( arg[2] ).."'", source, 255, 0, 0 )
				else
					mdata2 = "Object '"..arg[3].."'"
			...
			elseif ( arg[1] == "acl" ) then
				...
				if ( not aclGroupAddACL ( group, acl ) ) then
					action = nil
					outputChatBox ( "Error añadiendo ACL '"..tostring ( arg[3] ).."' al grupo '"..tostring ( arg[2] ).."'", source, 255, 0, 0 )
				else
					mdata2 = "ACL '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclobjects", arg[2] )
			...
			elseif ( arg[1] == "right" ) then
				...
				if ( not aclSetRight ( acl, right, enabled ) ) then
					action = nil
					outputChatBox ( "Error añadiendo permiso '"..tostring ( arg[3] ).."' al grupo '"..tostring ( arg[2] ).."'", source, 255, 0, 0 )
				else
					mdata2 = "Right '"..arg[3].."'"
		...
		if ( arg[3] ) then
			...
			if ( arg[1] == "object" ) then
				...
				if ( not aclGroupRemoveObject ( group, object ) ) then
					action = nil
					outputChatBox ( "Error - objeto '"..tostring ( object ).."' no existe en el grupo '"..tostring ( arg[2] ).."'", source, 255, 0, 0 )
				else
					mdata2 = "Object '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclobjects", arg[2] )
			...
			elseif ( arg[1] == "acl" ) then
				...
				if ( not aclGroupRemoveACL ( group, acl ) ) then
					action = nil
					outputChatBox ( "Error - ACL '"..tostring ( arg[3] ).."' no existe en el grupo '"..tostring ( arg[2] ).."'", source, 255, 0, 0 )
				else
					mdata2 = "ACL '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclobjects", arg[2] )
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] ["..getPlayerIP (source).."] removed "..mdata2)
			...
			elseif ( arg[1] == "right" ) then
				...
				if ( not aclRemoveRight ( acl, right ) ) then
					action = nil
					outputChatBox ( "Error - permiso '"..tostring ( arg[3] ).."' no existe en el ACL '"..tostring ( arg[2] ).."'", source, 255, 0, 0 )
				else
					mdata = "ACL '"..arg[2].."'"
					mdata2 = "Right '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclrights", arg[2] )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		elseif ( action == "ban" ) then
			...
			if bUseSerial then
				...
				outputChatBox ( "Baneaste la IP " .. getPlayerIP( player ), source, 255, 100, 70 )
				if isAnonAdmin then
					setTimer ( function()
		...
		elseif ( action == "setnick" )  then
			...
			if setPlayerName( player, data ) then
				outputChatBox ( "¡Cambiaste el nombre de '"..playername.."' a '"..data.."'!", source, 255, 100, 70 )
				outputChatBox ( "¡'"..getPlayerName( source ).."' cambió tu nombre a '"..data.."'!", player, 255, 100, 70 )
			else
		...
		elseif ( action == "setskin" ) then
			...
			if ( setElementModel( player, data) ) then
				...
				action = nil
				outputChatBox( "ID de skin inválido", source, 255, 0, 0 )
			end
		elseif ( action == "setmoney" ) then
			mdata = data
			if ( not setPlayerMoney ( player, data ) ) then
				outputChatBox ( "Información de dinero inválida", source, 255, 0, 0 )
				action = nil
			end
		...
		elseif ( action == "jetpack" ) then
			if ( doesPedHaveJetPack ( player ) ) then
				...
				if ( getPedOccupiedVehicle ( player ) ) then outputChatBox ( "Incapaz de dar jetpack a - "..getPlayerName ( player ).." , está en un vehículo", source, 255, 0, 0 )
				else
		...
		elseif ( action == "setgroup" ) then
			...
			if ( not isGuestAccount ( account ) ) then
				...
				if ( group ) then
					...
					outputChatBox ( "Error - Grupo admin no iniciado. Por favor reinstalar resource.", source, 255, 0 ,0 )
				end
			else
				outputChatBox ( "Error - el jugador no está conectado.", source, 255, 100 ,100 )
			end
		...
		elseif ( action == "warp" ) or ( action == "warpto" ) then
				...
				function warp ( p, to )
					...
					if ( i >= seats ) then
						warp ( p, to )
						outputConsole ( "El vehículo del jugador está lleno ("..getVehicleName ( vehicle ).." - asientos: "..seats..")", p )
					end
				else
					warp ( p, to )
				end
		...
		if ( vehicle ) then
			...
			elseif ( action == "setpaintjob" ) then
				...
				if ( not setVehiclePaintjob ( vehicle, data ) ) then
					action = nil
					outputChatBox ( "ID de paint job inválido", source, 255, 0, 0 )
				end
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		if ( action == "setgame" ) then
			if ( not setGameType ( tostring ( data ) ) ) then
				action = nil
				outputChatBox ( "Error ajustando el nombre del mapa.", source, 255, 0, 0 )
			end
		...
		elseif ( action == "settime" ) then
			if ( not setTime ( tonumber ( data ), tonumber ( data2 ) ) ) then
				action = nil
				outputChatBox ( "Error ajustando la hora.", source, 255, 0, 0 )
			end
			mdata = data..":"..data2
		...
		elseif ( action == "setpassword" ) then
			...
			elseif ( string.len ( data ) > 32 ) then
				outputChatBox ( "Fijar contraseña: 32 caracteres máximo", source, 255, 0, 0 )
			elseif ( not setServerPassword ( data ) ) then
				action = nil
				outputChatBox ( "Error fijando contraseña", source, 255, 0, 0 )
			end
			triggerEvent ( "aSync", source, "server" )
		elseif ( action == "setweather" ) then
			if ( not setWeather ( tonumber ( data ) ) ) then
				action = nil
				outputChatBox ( "Error fijando el clima.", source, 255, 0, 0 )
			end
		...
		elseif ( action == "setgamespeed" ) then
			if ( not setGameSpeed ( tonumber ( data ) ) ) then
				action = nil
				outputChatBox ( "Error ajustando la velocidad del juego.", source, 255, 0, 0 )
			end
		elseif ( action == "setgravity" ) then
			if ( setGravity ( tonumber ( data ) ) ) then
				...
				action = nil
				outputChatBox ( "Error ajustando la gravedad.", source, 255, 0, 0 )
			end
		elseif ( action == "setwaveheight" ) then
			if ( not setWaveHeight ( data ) ) then
				outputChatBox ( "Error fijando la altitud olas.", source, 255, 0, 0 )
				action = nil
			else
				mdata = data
			end
		elseif ( action == "setfpslimit" ) then
			if ( not setFPSLimit ( tonumber ( data ) ) ) then
				action = nil
				outputChatBox ( "Error ajustando el límite de FPS.", source, 255, 0, 0 )
			end
		else
			action = nil
		end
	...
	if ( action == "new" ) then
		...
		for _, p in ipairs ( getElementsByType ( "player" ) ) do
			if ( hasObjectPermissionTo ( p, "general.adminpanel" ) ) then
				outputChatBox( "Nuevo mensaje de " .. aReports[id].author .. " (" .. aReports[id].subject .. ")", p, 255, 0, 0 )
			end
		end
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		elseif ( action == "banserial" ) then
			...
			if ( isValidSerial ( data ) ) then
				...
				outputChatBox ( "Error - serial inválida", source, 255, 0, 0 )
				action = nil
			end
...
addEventHandler ( "aExecute", _root, function ( action, echo )
	...
	if ( hasObjectPermissionTo ( source, "command.execute" ) ) then
		...
		if ( echo == true ) then
			...
			outputChatBox( "¡Comando ejecutado! Resultó en: " ..restring, source, 0, 0, 255 )
		end
...
function checkNickOnChange(old, new)
	if aNickChangeTime[source] and aNickChangeTime[source] + tonumber(get("*nickChangeDelay")) > getTickCount() then
		cancelEvent()
		outputChatBox("Solo puedes cambiar tu nombre una vez cada "..(tonumber(get("*nickChangeDelay"))/1000).." segundos", source, 255, 0, 0)
		return false
	else
