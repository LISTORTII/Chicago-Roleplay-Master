...
function aPlayerScreenShot (player)
	if aScreenShotForm == nil then
		...
		aScreenShotForm		= guiCreateWindow	( x / 2 - 300, y / 2 - 125, 600, 250, "Administrador de capturas de pantalla", false )
		aScreenShotList		= guiCreateGridList	( 0.03, 0.08, 0.70, 0.90, true, aScreenShotForm )
		aScreenShotNew		= guiCreateButton	( 0.75, 0.08, 0.42, 0.09, "Tomar nueva", true, aScreenShotForm )
		aScreenShotDelete	= guiCreateButton	( 0.75, 0.18, 0.42, 0.09, "Eliminar", true, aScreenShotForm )
		aScreenShotView		= guiCreateButton	( 0.75, 0.28, 0.42, 0.09, "Ver", true, aScreenShotForm )
		aScreenShotRefresh	= guiCreateButton	( 0.75, 0.38, 0.42, 0.09, "Actualizar", true, aScreenShotForm )
		aScreenShotClose	= guiCreateButton	( 0.75, 0.88, 0.42, 0.09, "Cerrar", true, aScreenShotForm )
		guiGridListAddColumn(aScreenShotList,"Jugador",0.31 )
		guiGridListAddColumn(aScreenShotList,"Admin",0.31 )
		guiGridListAddColumn(aScreenShotList,"Fecha",0.27 )
...
addEventHandler("aClientScreenShot",resourceRoot,
	function (action,player,data,arg1,arg2,arg3)
		if action == "new" then
			...
			aScreenShotWindows[player].label = guiCreateLabel(0,0,1,1,"Cargando...",true,aScreenShotWindows[player].window)
			aScreenShotWindows[player].button = guiCreateButton(0.93,0.95,0.6,0.4,"Cerrar",true,aScreenShotWindows[player].window)
		...
		elseif action == "list" then
			...
			elseif action == "minimized" then
				guiSetText(aScreenShotWindows[player].label,"El jugador minimizó, intenta más tarde")
			elseif action == "disabled" then
				guiSetText(aScreenShotWindows[player].label,"El jugador no permite la toma de capturas")
			elseif action == "quit" then
				guiSetText(aScreenShotWindows[player].label,"El jugador abandonó")
			end
		end
	end
)
