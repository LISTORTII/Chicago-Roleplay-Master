...
function aReport ( )
	if ( aReportForm == nil ) then
		...
		aReportForm		= guiCreateWindow ( x / 2 - 150, y / 2 - 170, 300, 340, "Contactar admin", false )
					   guiCreateLabel ( 0.05, 0.11, 0.20, 0.07, "Categoría:", true, aReportForm )
					   guiCreateLabel ( 0.05, 0.19, 0.20, 0.07, "Asunto:", true, aReportForm )
					   guiCreateLabel ( 0.05, 0.34, 0.20, 0.07, "Mensaje:", true, aReportForm )
		aReportLblPlayer = guiCreateLabel ( 0.05, 0.27, 0.20, 0.07, "Jugador:", true, aReportForm )
		aReportBtnPlayer = guiCreateButton ( 0.75, 0.27, 0.20, 0.07, "Jugadores", true, aReportForm )
		aReportCategory	= guiCreateEdit ( 0.30, 0.10, 0.65, 0.07, "Pregunta", true, aReportForm )
...
function aReportSelectPlayer ( )
	if ( aSelectPlayer == nil ) then
		local x, y = guiGetScreenSize ( )
		aSelectPlayer = guiCreateWindow ( x / 2 - 155, y / 2 - 250, 310, 500, "Seleccionar jugador", false)
		...
		guiGridListAddColumn ( playerList, "Nombre del jugador", 0.85 )
		for _, player in pairs (getElementsByType("player")) do
			guiGridListSetItemText(playerList, guiGridListAddRow(playerList), 1, getPlayerName(player), false, false)
		end
		local btnSelectPlayer = guiCreateButton(0.57, 0.93, 0.33, 0.05, "Seleccionar", true, aSelectPlayer)
		...
		local btnClose = guiCreateButton(0.10, 0.93, 0.33, 0.05, "Cerrar", true, aSelectPlayer)
	...
	if ( button == "left" ) then
		if ( source == aReportAccept ) then
			if ( ( string.len ( guiGetText ( aReportSubject ) ) < 1 ) or ( string.len ( guiGetText ( aReportMessage ) ) < 5 ) ) then
				aMessageBox ( "Error", "Asunto/Mensaje faltante." )
			else
				local tableOut = {}
				...
				aMessageBox ( "Información", "Tu mensaje se ha enviado y será procesado tan pronto sea posible." )
