...
function aManageSettings ( resName )
	...
	if ( aSettingsForm == nil ) then
		...
		aSettingsTab	= guiCreateTab ( "Jugadores", aSettingsTabPanel, "players" )
		aSettingsList	= guiCreateGridList ( 0.03, 0.03, 0.94, 0.72, true, aSettingsTab )
						guiGridListAddColumn( aSettingsList, "nombre", 0.50 )
						guiGridListAddColumn( aSettingsList, "actual", 0.25 )
						guiGridListAddColumn( aSettingsList, "por defecto", 0.25 )
		aSettingsExit		= guiCreateButton ( 0.80, 0.93, 0.17, 0.04, "Cerrar", true, aSettingsTab )
	...
	guiSetText( aSettingsTab, resName .. " (ajustes)" )
	...
	if ( button == "left" ) then
		if ( source == aSettingsList ) then
			...
			if ( row ~= -1 ) then
				...
				aInputBox ( "Cambiar ajuste",
							"Ingresar nuevo valor para '".. friendlyname .."'",
							tostring(current),
