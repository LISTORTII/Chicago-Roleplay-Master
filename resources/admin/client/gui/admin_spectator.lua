...
function aSpectate ( player )
	if ( player == getLocalPlayer() ) then
		aMessageBox ( "Error", "¡No puedes espectarte a ti mismo!" )
		return
	end
...
function aSpectator.Initialize ()
	if ( aSpectator.Actions == nil ) then
		...
		aSpectator.Actions		= guiCreateWindow ( x - 190, y / 2 - 200, 160, 400, "Acciones", false )
		aSpectator.Ban			= guiCreateButton ( 0.10, 0.09, 0.80, 0.05, "Banear", true, aSpectator.Actions )
		aSpectator.Kick			= guiCreateButton ( 0.10, 0.15, 0.80, 0.05, "Kickear", true, aSpectator.Actions )
		aSpectator.Freeze			= guiCreateButton ( 0.10, 0.21, 0.80, 0.05, "Congelar", true, aSpectator.Actions )
		aSpectator.SetSkin		= guiCreateButton ( 0.10, 0.27, 0.80, 0.05, "Fijar skin", true, aSpectator.Actions )
		aSpectator.SetHealth		= guiCreateButton ( 0.10, 0.33, 0.80, 0.05, "Fijar salud", true, aSpectator.Actions )
		aSpectator.SetArmour		= guiCreateButton ( 0.10, 0.39, 0.80, 0.05, "Fijar armadura", true, aSpectator.Actions )
		aSpectator.SetStats		= guiCreateButton ( 0.10, 0.45, 0.80, 0.05, "Stats", true, aSpectator.Actions )
		aSpectator.Slap			= guiCreateButton ( 0.10, 0.51, 0.80, 0.05, "¡Golpear! "..aCurrentSlap.."hp", true, aSpectator.Actions )
		...
		aSpectator.Skip			= guiCreateCheckBox ( 0.08, 0.85, 0.84, 0.04, "Omitir jugadores muertos", true, true, aSpectator.Actions )
					  		  guiCreateLabel ( 0.08, 0.89, 0.84, 0.04, "____________________", true, aSpectator.Actions )
		aSpectator.Back			= guiCreateButton ( 0.10, 0.93, 0.80, 0.05, "Atrás", true, aSpectator.Actions )

		aSpectator.Players		= guiCreateWindow ( 30, y / 2 - 200, 160, 400, "Jugadores", false )
		...
		aSpectator.PlayerList		= guiCreateGridList ( 0.03, 0.07, 0.94, 0.92, true, aSpectator.Players )
					  		  guiGridListAddColumn( aSpectator.PlayerList, "Nombre del jugador", 0.85 )
		...
		aSpectator.Prev			= guiCreateButton ( x / 2 - 100, y - 50, 70, 30, "< Anterior", false )
		aSpectator.Next			= guiCreateButton ( x / 2 + 30,  y - 50, 70, 30, "Siguiente >", false )
...
function aSpectator.ClientDoubleClick ( button )
	if ( source == aSpectator.Slaps ) then
		if ( guiGridListGetSelectedItem ( aSpectator.Slaps ) ~= -1 ) then
			...
			guiSetText ( aTab1.Slap, "¡Golpear! "..aCurrentSlap.."hp" )
			guiSetText ( aSpectator.Slap, "¡Golpear! "..aCurrentSlap.."hp" )
		end
		guiSetVisible ( aSpectator.Slaps, false )
	end
end
...
function aSpectator.SwitchPlayer ( inc, arg, inc2 )
	...
	if ( #players <= 0 ) then
		aMessageBox ( "Pregunta", "Nadie para espectar. ¿Cerrar espectador?", "spectatorClose" )
		return
	end
	local current = 1
	...
	if ( next == current ) then
		aMessageBox ( "Pregunta", "Nadie más para espectar. ¿Cerrar espectador?", "spectatorClose" )
		return
	end
...
function aSpectator.Render ()
	...
	if ( not aSpectator.Spectating ) then
		dxDrawText ( "Nadie para espectar", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
	...
	if ( not x ) then
		dxDrawText ( "Error recibiendo coordenadas", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
	...
	local sx, sy = guiGetScreenSize ()
	dxDrawText ( "Espectando: "..getPlayerName ( aSpectator.Spectating ), sx - 170, 200, sx - 170, 200, tocolor ( 255, 255, 255, 255 ), 1 )
	if ( _DEBUG ) then
		...
		if ( isCursorShowing () ) then
			dxDrawText ( "Tip: mouse2 - alternar modo de cámara libre", 20, sy - 50, 20, sy - 50, tocolor ( 255, 255, 255, 255 ), 1 )
		else
			dxDrawText ( "Tip: usar desplazamiento del mouse para acerca/alejar", 20, sy - 50, 20, sy - 50, tocolor ( 255, 255, 255, 255 ), 1 )
		end
	end
end
