...
function aViewMessage ( id )
	...
	if ( aViewMessageForm == nil ) then
		...
		aViewMessageForm	= guiCreateWindow ( x / 2 - 150, y / 2 - 129, 300, 258, "", false )
					   guiCreateLabel ( 0.05, 0.10, 0.30, 0.09, "Categoría:", true, aViewMessageForm )
					   guiCreateLabel ( 0.05, 0.18, 0.30, 0.09, "Hora:", true, aViewMessageForm )
					   guiCreateLabel ( 0.05, 0.26, 0.30, 0.09, "Por:", true, aViewMessageForm )
		aViewMessageLabelSuspect = guiCreateLabel ( 0.05, 0.34, 0.30, 0.09, "Sospechoso:", true, aViewMessageForm )
		aViewMessageSuspect	= guiCreateButton ( 0.40, 0.34, 0.55, 0.07, "Ver información", true, aViewMessageForm )
		...
		aViewMessageCloseB	= guiCreateButton ( 0.77, 0.86, 0.20, 0.09, "Cerrar", true, aViewMessageForm )
	...
	if ( button == "left" ) then
		if ( source == aViewMessageSuspect ) then
			if ( aSuspectInfo == nil ) then
				...
				if ( suspectInfo ) then
					local x, y = guiGetScreenSize()
					aSuspectInfo = guiCreateWindow(x / 2 - 145, y / 2 - 192.5, 290, 385, "Información del jugador", false)
					local btnClose = guiCreateButton(0.365, 0.88, 0.27, 0.10, "Cerrar", true, aSuspectInfo)
					addEventHandler("onClientGUIClick", btnClose, function()
						destroyElement(aSuspectInfo)
						aSuspectInfo = nil
					end, false)

					local infoMemo = guiCreateMemo(0.04, 0.1, 0.96, 0.75, "Apodo: "..suspectInfo.name.."\nNombre de cuenta: "..suspectInfo.username.."\nIP: "
						..suspectInfo.ip.."\nSerial: "..suspectInfo.serial.."\nVersión de MTA: "..suspectInfo.version.."\n\nRegistro del chat:\n"..suspectInfo.chatLog,
					true, aSuspectInfo)
					guiMemoSetReadOnly(infoMemo, true)
				else
					aMessageBox ( "Error", "Este reporte no contiene información del sospechoso." )
				end
			end
		end
	end
