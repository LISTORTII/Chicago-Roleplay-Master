...
function aVehicleCustomize ( player )
	if ( aVehicleForm == nil ) then
		...
		aVehicleForm		= guiCreateWindow ( x / 2 - 300, y / 2 - 150, 600, 450, "Customizaciones del vehículo", false )
		...
		for i = 1, 17 do
			if ( i ~= 12 ) then
				...
				guiGridListAddColumn( aVehicleUpgrades[c].list, "ID de mejora", 0.90 )
				guiSetVisible ( aVehicleUpgrades[c].list, false )
				c = c + 1
			end
		end

		aVehicleUpgradeAll	= guiCreateButton ( 0.04, 0.92, 0.15, 0.05, "Mejorar todo", true, aVehicleForm )
		aVehicleRemoveAll	= guiCreateButton ( 0.20, 0.92, 0.15, 0.05, "Remover todo", true, aVehicleForm )
		aVehicleUpgrade	= guiCreateButton ( 0.375, 0.92, 0.20, 0.05, "Seleccionados", true, aVehicleForm )
		...
		aVehiclePaintjobSet	= guiCreateButton ( 0.90, 0.10, 0.07, 0.048, "Fijar", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.15, 0.15, 0.05, "Colores:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.20, 0.15, 0.05, "Color 1:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.25, 0.15, 0.05, "Color 2:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.30, 0.15, 0.05, "Color 3:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.35, 0.15, 0.05, "Color 4:", true, aVehicleForm )
		...
		aVehicleColorSet	= guiCreateButton ( 0.84, 0.41, 0.14, 0.05, "Fijar", true, aVehicleForm )
		guiCreateLabel ( 0.63, 0.5, 0.15, 0.05, "Color de luces:", true, aVehicleForm )
		aLightsColor = guiCreateEdit ( 0.79, 0.5, 0.13, 0.048, "#ffffff", true, aVehicleForm ) guiEditSetMaxLength ( aLightsColor, 7 )
		aLightsColorSet	= guiCreateButton ( 0.93, 0.5, 0.05, 0.05, "Fijar", true, aVehicleForm )
		aVehicleUpgradeNames = guiCreateCheckBox ( 0.63, 0.7, 0.30, 0.04, "Mostrar nombres de mejoras", false, true, aVehicleForm )
					   if ( aGetSetting ( "aVehicleUpgradeNames" ) ) then guiCheckBoxSetSelected ( aVehicleUpgradeNames, true ) end
		aVehicleClose		= guiCreateButton ( 0.86, 0.92, 0.19, 0.05, "Cerrar", true, aVehicleForm )
		...
		guiCreateLabel ( 0.63, 0.61, 0.15, 0.05, "Placa:", true, aVehicleForm )
		aLicensePlate = guiCreateEdit ( 0.79, 0.61, 0.13, 0.048, "", true, aVehicleForm ) guiEditSetMaxLength ( aLicensePlate, 8 )
		aLicensePlateSet = guiCreateButton ( 0.93, 0.61, 0.05, 0.05, "Fijar", true, aVehicleForm )


		aVehicleColorClose	= guiCreateButton ( 0.86, 0.86, 0.19, 0.15, "Cerrar", true, aVehicleColorForm )
	...
	if ( vehicle ) then
		...
		guiSetText ( aVehicleForm, "Customizaciones del vehículo ("..tostring ( getVehicleName ( vehicle ) )..")" )
...
function aVehicleCheckUpgrades ( vehicle )
	if ( vehicle ) then
		...
		outputChatBox ( "Debes estar dentro de un vehículo.", 255, 0, 0 )
	end
end
...
function aClientVehicleClick ( button )
	...
	if ( button == "left" ) then
		...
		elseif source == aVehicleColor1 then
			openPicker("vehicleColor1", guiGetText(aVehicleColor1), "Fijar color del vehículo")
		elseif source == aVehicleColor2 then
			openPicker("vehicleColor2", guiGetText(aVehicleColor2), "Fijar color del vehículo")
		elseif source == aVehicleColor3 then
			openPicker("vehicleColor3", guiGetText(aVehicleColor3), "Fijar color del vehículo")
		elseif source == aVehicleColor4 then
			openPicker("vehicleColor4", guiGetText(aVehicleColor4), "Fijar color del vehículo")
		elseif source == aLightsColor then
			openPicker("lightsColor", "#ffffff", "Fijar color de las luces")
