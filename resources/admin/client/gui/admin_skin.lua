...
function aPlayerSkin ( player )
	if ( aSkinForm == nil ) then
		...
		aSkinForm		= guiCreateWindow ( x / 2 - 140, y / 2 - 125, 280, 250, "Seleccionar skin del jugador", false )
		aSkinLabel		= guiCreateLabel ( 0.03, 0.09, 0.94, 0.07, "Selecciona un skin de la lista o ingresa el ID", true, aSkinForm )
					  guiLabelSetHorizontalAlign ( aSkinLabel, "center" )
					  guiLabelSetColor ( aSkinLabel, 255, 0, 0 )
		aSkinGroups		= guiCreateCheckBox ( 0.03, 0.90, 0.70, 0.09, "Ordenar por grupos", false, true, aSkinForm )
		...
		aSkinAccept		= guiCreateButton ( 0.75, 0.28, 0.27, 0.09, "Elegir", true, aSkinForm, "setskin" )
		aSkinCancel		= guiCreateButton ( 0.75, 0.88, 0.27, 0.09, "Cancelar", true, aSkinForm )
		...
		if ( source == aSkinAccept ) then
			if ( tonumber ( guiGetText ( aSkinID ) ) ) then
				...
				if ( guiGridListGetSelectedItem ( aSkinList ) ~= -1 ) then
					...
					aMessageBox ( "Advertencia", "Jugador no seleccionado/ID inválido." )
				end
			end
