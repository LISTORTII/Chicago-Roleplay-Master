...
function aManageACL ()
	if ( aAclForm == nil ) then
		...
		aAclForm		= guiCreateWindow ( x / 2 - 230, y / 2 - 250, 460, 500, "Administrar ACL", false )
		...
		aACLCreateGroup	= guiCreateButton ( 0.55, 0.05, 0.40, 0.04, "Crear grupo", true, aAclForm )
		aACLCreateACL		= guiCreateButton ( 0.55, 0.10, 0.40, 0.04, "Crear ACL", true, aAclForm )
		...
		aACLDestroyGroup	= guiCreateButton ( 0.55, 0.25, 0.40, 0.04, "Destruir grupo", true, aAclForm )
		aACLDestroyACL	= guiCreateButton ( 0.55, 0.25, 0.40, 0.04, "Destruir ACL", true, aAclForm )
		aACLAddObject		= guiCreateButton ( 0.55, 0.30, 0.40, 0.04, "Añadir objeto", true, aAclForm )
		aACLRemoveObject	= guiCreateButton ( 0.55, 0.35, 0.40, 0.04, "Remover objeto", true, aAclForm )
		aACLAddACL		= guiCreateButton ( 0.55, 0.40, 0.40, 0.04, "Añadir ACL", true, aAclForm )
		aACLRemoveACL	= guiCreateButton ( 0.55, 0.45, 0.40, 0.04, "Remover ACL", true, aAclForm )
		...
		aACLCancel		= guiCreateButton ( 0.76, 0.40, 0.19, 0.04, "Cancelar", true, aAclForm )

		aACLAddRight		= guiCreateButton ( 0.55, 0.30, 0.40, 0.04, "Añadir permiso", true, aAclForm )
		aACLExit		= guiCreateButton ( 0.75, 0.90, 0.27, 0.04, "Cerrar", true, aAclForm )
		aclDisplayOptions ( "", "" )
...
function aAdminACL ( type, acltable )
	...
	if ( type == "aclgroups" ) then
		...
		guiGridListSetItemText ( aACLList, aAclData["group_row"], 2, "Grupos:", true, false )
	...
	elseif ( type == "aclobjects" ) then
		...
		for i, group in ipairs ( aAclData["groups"] ) do
			...
			if ( group == acltable["name"] ) then
				aclDisplayOptions ( "Grupo", acltable["name"] )
				aAclData["objects_row"] = guiGridListAddRow ( aACLList )
				aAclData["group_objects"][group] = acltable["objects"]
				guiGridListSetItemText ( aACLList, aAclData["objects_row"], 2, "  objetos:", true, false )
	...
	elseif ( type == "aclrights" ) then
		...
		for i, acl in ipairs ( aAclData["acl"] ) do
			...
			if ( acl == acltable["name"] ) then
				...
				guiGridListSetItemText ( aACLList, aAclData["rights_row"], 2, "  permisos:", true, false )
...
function aClientACLClick ( button )
	...
	if ( button == "left" ) then
		...
		elseif ( source == aACLCreateGroup ) then
			aInputBox ( "Crear grupo ACL", "Ingresar nombre del grupo:", "", "aclCreateGroup" )
		elseif ( source == aACLCreateACL ) then
			aInputBox ( "Crear ACL", "Ingresar nombre del ACL:", "", "aclCreate" )
		elseif ( source == aACLAddObject ) then
			aInputBox ( "Crear objeto en el ACL", "Ingresar nombre del objeto:", "", "aclAddObject", aAclData["current"] )
		elseif ( source == aACLAddRight ) then
			aInputBox ( "Crear permiso en el ACL", "Ingresar nombre del permiso:", "", "aclAddRight", aAclData["current"] )
		elseif ( source == aACLDestroyGroup ) then
			aMessageBox ( "Advertencia", "¿Seguro de querer destruir el grupo "..aAclData["current"].." ?", "aclDestroyGroup", aAclData["current"])
		elseif ( source == aACLDestroyACL ) then
			aMessageBox ( "Advertencia", "¿Seguro de querer destruir el ACL "..aAclData["current"].." ?", "aclDestroy", aAclData["current"])
