...
function aViewMessages ( player )
	if ( aMessagesForm == nil ) then
		...
		aMessagesForm	= guiCreateWindow ( x / 2 - 250, y / 2 - 125, 620, 250, "Ver mensajes", false )

		aMessagesList		= guiCreateGridList ( 0.02, 0.09, 0.82, 0.85, true, aMessagesForm )
					   guiGridListAddColumn( aMessagesList, "#", 0.08 )
					   guiGridListAddColumn( aMessagesList, "Asunto", 0.46 )
					   guiGridListAddColumn( aMessagesList, "Fecha", 0.23 )
					   guiGridListAddColumn( aMessagesList, "Autor", 0.19 )
		aMessagesRead		= guiCreateButton ( 0.86, 0.15, 0.12, 0.09, "Leer", true, aMessagesForm )
		aMessagesDelete		= guiCreateButton ( 0.86, 0.25, 0.12, 0.09, "Eliminar", true, aMessagesForm )
		aMessagesBanSerial	= guiCreateButton ( 0.86, 0.40, 0.12, 0.09, "Ban. serial", true, aMessagesForm )
		aMessagesBanIP		= guiCreateButton ( 0.86, 0.50, 0.12, 0.09, "Banear IP", true, aMessagesForm )
		aMessagesRefresh	= guiCreateButton ( 0.86, 0.65, 0.12, 0.09, "Actualizar", true, aMessagesForm )
		aMessagesClose		= guiCreateButton ( 0.86, 0.85, 0.12, 0.09, "Cerrar", true, aMessagesForm )
...
function aClientMessagesClick ( button )
	if ( button == "left" ) then
		...
		elseif ( source == aMessagesRead ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "Advertencia", "¡Mensaje no seleccionado!", nil )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
				aViewMessage ( tonumber ( id ) )
			end
		elseif ( source == aMessagesDelete ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "Advertencia", "¡Mensaje no seleccionado!" )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
		...
		elseif ( source == aMessagesBanSerial ) then
			...
			aInputBox ( "Banear serial", "Ingresar la serial a banear:", data.suspect.serial, "banSerial", _, _, data.suspect.name, data.category )
		elseif ( source == aMessagesBanIP ) then
			local data = _messages[tonumber ( guiGridListGetItemText ( aMessagesList, guiGridListGetSelectedItem ( aMessagesList ), 1 ) )]
			aInputBox ( "Banear IP", "Ingresar la IP a banear:", data.suspect.ip, "banIP", _, _, data.suspect.name, data.category )
