	...
	--if not source:getAccount():isGuest () then
		...
		if _type == 0 then
			if message ~= "" and message ~= " " and message:len() >= 1 then
				...
				--[[source:setData("TextInfo", {message3, 221, 250, 255})
				setTimer(function(p)
					if isElement(p) then
				...
				for _,v in ipairs(nearPlayers) do
					v:outputChat("#FFFCD1"..nick.." dice en "..(source:getData("Roleplay:Idioma") or "Ingles")..":#FFFFFF "..message3.."", 221, 250, 255, true)
				end
				for i,p in ipairs(Element.getAllByType("player")) do
					if p:getData("espiando") == i then
						p:outputChat("#FFFCD1"..nick.." dice en "..(source:getData("Roleplay:Idioma") or "Ingles")..":#FFFFFF "..message3.."", 221, 250, 255, true)
					end
				end
		...
		if _type == 1 then
			if message ~= "" and message ~= " " and message:len() >= 1 then
				...
				for _,v in ipairs(nearPlayers) do
					v:outputChat("#FF00D8 *"..nick.." "..message.."", 215, 122, 8, true)
				end
				if (not antiSpam[source]) then
					antiSpam[source] = {}
				end
...
function chatSUSURRO( source, cmd, ... )
	if not source:getAccount():isGuest () then
		...
		if message ~= "" and message ~= " " and message:len() >= 3 then
			...
			for _,v in ipairs(nearPlayers) do
				v:outputChat("#EDEEE1["..(source:getData("Roleplay:Idioma") or "Ingles").."] "..nick.." susurra: "..message.."", 165, 242, 255, true)
			end
...
function chatAme(source, cmd, ...)
	if not source:getAccount():isGuest () then
		...
		if message ~= "" and message ~= " " and message:len() >= 1 then
			...
			chatCol = ColShape.Sphere(x, y, z, 2)
			nearPlayers = chatCol:getElementsWithin("player") 
			source:outputChat("#FF00D8> "..message.."", 165, 242, 255, true)
			source:setData("TextInfo", {"> "..message, 255, 0, 216})
