...
addEventHandler("onClientRender", getRootElement(), function()
	...
	if not isPlayerMapVisible() then
		...
		dxDrawText("Chicago Roleplay 1.0", 395*sx, 741*sy, 1358*sx, 314*sy, tocolor(255, 255, 255, 120), 1.00, "default", "right", "top", false, false, false, false, false)
		...
		if hudPlayer == true then
			...
			dxDrawText("/yo : ["..tostring(localPlayer:getData("yo")[1]).."]", 10*sx, 730*sy, 0*sx, 314*sy, tocolor(255, 255, 255, 200), 1.00, "default-bold", "left", "top", false, false, false, false, false)



			dxDrawText(" "..dia.." / ".. meses[(mes+1)][1].." / "..ano.." | "..hora.." : "..minutos.." (GMT) | "..localPlayer:getPing().." ms | "..(localPlayer:getData("FPS") or 0).." | "..localPlayer:getName().." [ ID:"..(localPlayer:getData("ID") or 0).." ] ", 10*sx, 750*sy, 0*sx, 314*sy, tocolor(255, 255, 255, 200), 1, "default-bold", "left", "top", false, false, false, false, false)
			...
			if ( state == "sprint" or state == "jump" or loadingSprint == true ) then
				...
				if stamina then
					...
					dxDrawRectangle( 1165*sx, 40*sy, 123*sx, 9*sy, tocolor(252, 3, 3, 100), false )



					dxDrawRectangle( 1165*sx, 40*sy, 122*sx*(stamina/100), 9*sy, tocolor(252, 248, 3, 255), false )
...
setTimer(function()
	for i,player in ipairs(getElementsByType("player")) do
		local comida = player:getData("Comida") or 100
		if comida > 0 then
			player:setData("Comida",comida -1)
		end
	end
end,800000,0) --
...
setTimer(function()
	for i,player in ipairs(getElementsByType("player")) do
		local agua = player:getData("Agua") or 100
		if agua > 0 then
			player:setData("Agua",agua -1)
		end
	end
end,700000,0) -- 
