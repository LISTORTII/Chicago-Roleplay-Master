local pickupDNI = Pickup(359.0595703125, 168.12109375, 1008.3828125, 3, 1581, 0)
...
local pickupTarjeta = Pickup(361.830078125, 173.8525390625, 1008.3828125, 3, 1581, 0)
	...
	if source:getInterior() == 3 and source:getDimension() == 0 then
		if isElementWithinPickup(source, pickupTarjeta) then
			if source:getMoney() >= 60 then
				...
				local dni = math.random(40000000,50000000)
				if s == 0 then
					--source:triggerEvent("tarjeta",source)
					source:outputChat("Ahora tienes una tarjeta de Credito", 255, 255, 255, true)
