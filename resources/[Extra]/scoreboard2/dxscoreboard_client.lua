	...
	if #scoreboardColumns ~= 0 then
				...
				for key, team in ipairs( teams ) do
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							teamName = teamName .. " (" .. teamMemberCount .. " jugador" .. iif( teamMemberCount == "1", "", "es" ) .. ")"
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				...
				dxDrawText( "Chicago Roleplay 1.0", topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cWhite, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "center", "top", false, false, drawOverGUI )
			...
			if serverInfo.players and showServerInfo then
				...
				scoreboardRemoveColumn(Country)

				local text = "Jugadores: " .. tostring( #players ) .. "/" .. serverInfo.players
			...
			local textLength = dxGetTextWidth( "Chicago Roleplay 1.0", fontscale(rmbFont, s(0.75)), rmbFont )

			local textHeight = dxGetFontHeight( fontscale(rmbFont, s(0.75)), rmbFont )

			dxDrawText( "Chicago Roleplay 1.0", sX/2-(textLength/2), topY+scoreboardDimensions.height-textHeight-s(2), sX/2+(textLength/2), topY+scoreboardDimensions.height-s(2), cWhite, fontscale(serverInfoFont, s(0.75)), rmbFont, "left", "top", false, false, drawOverGUI )
